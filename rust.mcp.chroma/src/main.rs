...
use clap::Parser;
use config::Config;
use mcp_server::{ByteTransport, Server, router::Router, router::RouterService};
use mcp_spec::{
    content::Content,
...
impl ChromaRouter {
    ...
    async fn call_tool_method<T, R, F, Fut>(
        &self,
        args: Value,
        f: F,
    ...
    async fn dispatch_method(&self, name: &str, args: Value) -> Result<Value, anyhow::Error> {
        match name {
            "chroma_list_collections" => {
                self.call_tool_method(args, tools::chroma_list_collections)
                    .await
            }
            "chroma_create_collection" => {
                self.call_tool_method(args, tools::chroma_create_collection)
                    .await
            }
            "chroma_peek_collection" => {
                self.call_tool_method(args, tools::chroma_peek_collection)
                    .await
            }
            "chroma_get_collection_info" => {
                self.call_tool_method(args, tools::chroma_get_collection_info)
                    .await
            }
            "chroma_get_collection_count" => {
                self.call_tool_method(args, tools::chroma_get_collection_count)
                    .await
            }
            "chroma_modify_collection" => {
                self.call_tool_method(args, tools::chroma_modify_collection)
                    .await
            }
            "chroma_delete_collection" => {
                self.call_tool_method(args, tools::chroma_delete_collection)
                    .await
            }
            "chroma_add_documents" => {
                self.call_tool_method(args, tools::chroma_add_documents)
                    .await
            }
            "chroma_query_documents" => {
                self.call_tool_method(args, tools::chroma_query_documents)
                    .await
            }
            "chroma_query_documents_with_translation" => {
                self.call_tool_method(args, tools::chroma_query_documents_with_translation)
                    .await
            }
            "chroma_get_documents" => {
                self.call_tool_method(args, tools::chroma_get_documents)
                    .await
            }
            "chroma_update_documents" => {
                self.call_tool_method(args, tools::chroma_update_documents)
                    .await
            }
            "chroma_delete_documents" => {
                self.call_tool_method(args, tools::chroma_delete_documents)
                    .await
            }
            "chroma_smart_add_documents" => {
                self.call_tool_method(args, tools::chroma_smart_add_documents)
                    .await
            }
            "process_thought" => self.call_tool_method(args, tools::process_thought).await,
...
async fn run_server(
    transport: ByteTransport<tokio::io::Stdin, tokio::io::Stdout>,
    config: Config,
) -> Result<()> {
