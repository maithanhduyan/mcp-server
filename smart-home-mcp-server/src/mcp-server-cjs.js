...
/**
 * Smart Home MCP Server (CommonJS Version)
...
// Import controllers based on environment - simplified approach
let GpioController;

// Check if we're on a Raspberry Pi environment
...
if (isRaspberryPi) {
  try {
    ...
    console.log('Using real GPIO controller (Raspberry Pi mode)');
  } catch (error) {
    console.log('GPIO controller failed, falling back to mock');
    GpioController = require('./controllers/mockGpioController.js');
  }
} else {
  // Use mock controller for development/Windows
...
module.exports = { SmartHomeMCPServer };
